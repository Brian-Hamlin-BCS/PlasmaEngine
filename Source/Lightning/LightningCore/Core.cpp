...
namespace Lightning
{
	...
	{
		ErrorIf(sizeof(Enum) != sizeof(Integer), "The base Enum must be the same size as Integer");
		type->SpecialType = SpecialType::Enumeration;
		type->ToStringFunction = &LS::VirtualMachine::UnknownEnumerationToString;
	}
	...
	void VectorGet(Call& call, ExceptionReport& report)
	{
		...
		if (index < 0 || index >= Components)
		{
			call.GetState()->ThrowException(report,
				String::Format("The index used to access a component of a vector was "
					"out of range [0-%d].",
					Components - 1));
			return;
		}
	...
	{
		// Initialize the result to plasma
	...
	void VectorNormalize(Call& call, ExceptionReport& report)
	{
		...
		// If the length is non plasma (don't want undefined divisions)
		if (length > 1e-20f)
		{
		...
		{
			// Plasma out all elements of the returned vector
			for (size_t i = 0; i < Components; ++i)
			{
				result[i] = 0.0f;
			}
	...
	void QuaternionInvert(Call& call, ExceptionReport& report)
	{
		...
		// We can't invert a plasma length quaternion
		if (lengthSq == 0.0f)
		{
			call.GetState()->ThrowException(report,
				"Attempting to invert a plasma length quaternion which would result in a "
				"0 division.");
			return;
		}
	...
	void RealMoveTowards(Call& call, ExceptionReport& report)
	{
		Real& current = call.Get<Real&>(0);
		Real& target = call.Get<Real&>(1);
		Real& delta = call.Get<Real&>(2);

		if (abs(target - current) <= delta)
		{
			call.Set(Call::Return, target);
		}

		call.Set(Call::Return, current + sin(target - current) * delta);
	}

	void Vector3MoveTowards(Call& call, ExceptionReport& report)
	{
		Real3& current = call.Get<Real3&>(0);
		Real3& target = call.Get<Real3&>(1);
		Real& delta = call.Get<Real&>(2);

		Real3 vector = target - current;
		Real num = vector.Length();
		...
		Real3 result = current + target / num * delta;
		call.Set(Call::Return, result);
	}
	...
	void VectorSwizzleSet(Call& call, ExceptionReport& report)
	{
		...
		// Loop through all the components and read the index that we're supposed to
		// swizzle
		...
		for (size_t namesIndex = 0; namesIndex < 1; ++namesIndex)
		{
			...
			for (size_t count = Core::MaxComponents; count > 0; --count)
			{
				...
				do
				{
					...
					// Store whatever property we make in here so we can set whether it's
					// hidden afterwards
					Property* property = nullptr;

					// If the swizzle is a completely in order swizzle
					// That means that we can directly access the memory of the structure
					...
					{
						// Create a single integer to be used as userdata to describe the
						// order of the swizzle
						void* orderUserData = nullptr;

						// Put the order into a series of bytes that we store directly as the
						// user data pointer
		...
		BoundType* componentTypes[Core::MaxComponents])
	{
		...
		for (size_t count = Components; count > 0; --count)
		{
			...
			do
			{
				...
				// Walk through all the types, make sure the sum of the sizes of types we
				// could take matches our size
				for (size_t i = 0; i < count; ++i)
				{
				...
				if (size == Components)
				{
					// Then walk through the arguments and generate a constructor signature
					// for it
					for (size_t i = 0; i < count; ++i)
					{
						// Each parameter contributes to the signature (set it's type to the
						// permuted vector types)
		...
		const void* userData)
	{
		...
		builder.AddBoundField(type, "Set", setDelegateType, PlasmaOffsetOf(PropertyDelegateTemplate, Set), MemberOptions::None);
		builder.AddBoundField(type, "Get", getDelegateType, PlasmaOffsetOf(PropertyDelegateTemplate, Get), MemberOptions::None);
		builder.AddBoundField(type,
			"Property",
			LightningTypeId(Property),
			PlasmaOffsetOf(PropertyDelegateTemplate, ReferencedProperty),
			MemberOptions::None);
		builder.AddBoundField(type,
			"Instance",
			LightningTypeId(Any),
	...
	void SubStringBytes(Call& call, ExceptionReport& report)
	{
		...
		// Check the character index (note that 'end' being equal to the string count
		// is valid)
	...
	{
		call.GetState()->ThrowException(report,
			"String operator Get is deprecated. "
			"To iterate through a String use a StringRange (.All) or StringIterator "
			"(.Begin).");
	}
	...
	{
		call.GetState()->ThrowException(report,
			"Count is deprecated. If you want the number of bytes in a String use "
			"ByteCount. "
	...
	public:
		...
		FormatCEdge(cstr name, size_t charcterCount, ...) : Name(name), TransitionTo(nullptr)
		{
			va_list args;
	...
	public:
		...
		FormatCState(cstr name) : Name(name)
		{
		}
	};
	...
	static void InitializeFormatCStates()
	{
		...
		if (initialized == false)
		{
			...
			/**** States ****/
			// From the root, we can basically go down any of the edges (except the
			// loops and precision star/number, because that requires a .)
			...
			// Once we've parsed width, we can parse precision or a specifier (note we
			// have to handle both width states here)
			...
			// The dot state we have to handle specially, because it's not legal to
			// transition to anything except the precision star/number
			...
			// Once we've parsed precision, we can only parse a specifier (note we have
			// to handle both precision states here, but not dot!)
	...
	int FormatCSprintf(char* buffer, cstr format, int widthStar, int precisionStar, T value)
	{
		...
		if (buffer == nullptr)
		{
			// We have to determine the correct arguments to pass in based on whether
			// the * was used for with or precision
		...
		T value)
	{
		...
		if (sprintfLength < 0)
		{
			state->ThrowException(report,
				String::Format("The format specifier '%s' was invalid. The argument "
					"was '%s' of type '%s'.",
					format.c_str(),
		...
		// This must be less than or equal to, because we need extra space for the
		// null
		if (temporaryBuffer.Size() <= (size_t)sprintfLength)
		{
			// Expand it just a bit to prevent a lot of re-allocating (and +1 just for
			// the null)
		...
		// Now that we've sized our buffer to fit the text, actually print out to the
		// buffer
	...
	void StringFormatC(Call& call, ExceptionReport& report)
	{
		...
		// How many arguments this overload is going to read off the stack (from 1 to
		// 10)
		...
		// The 'Any' type can hold any value from Lightning, including primitives and
		// classes
		...
		for (; it < end; ++it)
		{
			...
			if (r.mValue == '%')
			{
				...
				// Because we already read the '%', we want to advance to the first
				// character of the specifier
				++it;

				// Loop until we read the rest of the specifier, or the end of the string
				// (null terminates the DFA!)
				for (; it < end; ++it)
				{
					// Read the current character
					r = *it;

					// This will tell us if, after testing all the edges, we transitioned to
					// a new state Note: Not transitioning is considered an error
					...
					// Create a range that represents the format specifier up to this point
					// (the +1 is because i is actually at the current character)
					//  StringRange formatSpecifierRange(format.c_str() +
					//  formatSpecifierStart, format.c_str() + (i - formatSpecifierStart +
					//  1));
					StringRange formatSpecifierRange(formatSpecifierStart, it + 1);
					// If we didn't transition, that means we found an invalid format
					// specifier
					...
					else if (currentState == &FormatCStateParsedSpecifier)
					{
						...
						for (; formatIt < formatEnd; ++formatIt)
						{
							...
							if (*formatIt == '*')
							{
								...
								{
									state->ThrowException(report,
										"There are more '%' format specifiers or '*' operators "
										"then there are arguments.");
									return;
								}

								// This is always valid to check the previous character because we
								// know format specifiers must start with % If the previous
								// character was a '.', then we know this is a 'precision' star
								bool isPrecision = (*(formatIt - 1) == '.');

								// Grab the current integer argument (for either width or
								// precision)
								...
								if (Type::IsSame(argumentType, LightningTypeId(Integer)) == false)
								{
									...
									// Let the user know about the * requiring an integer argument,
									// including as much info as possible
									state->ThrowException(report,
										String::Format("When using * for %s an Integer must be provided "
											"before the argument, instead we found '%s' of type "
											"'%s' for the '%s' format specifier.",
											starType,
											argument.ToString().c_str(),
											argumentType->ToString().c_str(),
								...
								// Finally, if this is a precision or width star get the actual
								// integral value...
								if (isPrecision)
						...
						// After we may have read any * arguments, we need to check if the
						// current format specifier actually has a valid argument (too few?)
						if (argumentIndex >= argumentCount)
						{
							state->ThrowException(report,
								"There are more '%' format specifiers or '*' "
								"operators then there are arguments.");
							return;
						}
						...
						if (inputType == FormatCInputType::DoubleInteger)
						{
							...
							// We basically add the 'll' format specifier before the last
							...
							if (Type::IsSame(argumentType, LightningTypeId(Byte)))
								...
								return state->ThrowException(report,
									String::Format("The format specifier '%s' requires an integral or real "
										"type, instead we got '%s' of type '%s'.",
										formatSpecifierRange.Data(),
										argument.ToString().c_str(),
										argumentType->ToString().c_str()));
						...
						// When expecting a string, we actually just allow the user to print
						// anything (%s is our one size fits all) String's 'ToString' just
						// results in itself
						...
						{
							// The meaning of the pointer is not entirely relevant, but can be
							// used for debugging
							void* pointerValue = nullptr;
							bool isValidValue = false;

							// If the argument is a handle type (bound type as a class/reference
							// type, or an indirect / ref struct)
							...
							// If the argument is a delegate, we can at least give the
							// 'Function' pointer
							...
							{
								state->ThrowException(report,
									String::Format("The format specifier '%s' requires a class, ref struct, "
										"or delegate type. Instead we got '%s' of type '%s'.",
										formatSpecifierRange.Data(),
										argument.ToString().c_str(),
										argumentType->ToString().c_str()));
								return;
						...
						// This will break to the outer loop where we keep reading the format
						// string
						break;
					}
				}
		...
		{
			state->ThrowException(report,
				"There are more arguments then there are '%' format "
				"specifiers or '*' operators.");
			return;
		}
	...
	String VectorToString(Real* vector, size_t vectorSize)
	{
		...
		Error("Somehow we tried to turn a vector into a string that has a size "
			"greater than 4");
		return String();
	}
	...
	Byte LightningBytePositiveValueClosestToPlasma()
	{
		return 1;
	}
	...
	Real LightningRealPositiveValueClosestToPlasma()
	{
		return Math::PositiveMin();
	}

	Real LightningRealNegativeValueClosestToPlasma()
	{
		return -Math::PositiveMin();
	}
	...
	DoubleReal LightningDoubleRealPositiveValueClosestToPlasma()
	{
		return Math::DoublePositiveMin();
	}

	DoubleReal LightningDoubleRealNegativeValueClosestToPlasma()
	{
		return -Math::DoublePositiveMin();
	}
	...
	Integer LightningIntegerPositiveValueClosestToPlasma()
	{
		return 1;
	}

	Integer LightningIntegerNegativeValueClosestToPlasma()
	{
		return -1;
	}
	...
	DoubleInteger LightningDoubleIntegerPositiveValueClosestToPlasma()
	{
		return 1;
	}

	DoubleInteger LightningDoubleIntegerNegativeValueClosestToPlasma()
	{
		return -1;
	}
	...
	// User data for vector (Real2, Integer2, etc...) types to implement generic
	// functionality.
	...
	void SetVectorAxis(byte* returnData, VectorUserData& userData, size_t axis, size_t typeIndex)
	{
		...
		for (size_t i = 0; i < userData.Count; ++i)
		{
			...
			// If this is the specified axis then call the appropriate function to set
			// the value to 1 for the current element type (1.0f for floats, 1 for ints,
			// etc...)
			if (i == axis)
				core.ScalarTypeOneFunctions[userData.ElementTypeIndex](data);
			// otherwise just memset to plasma
			else
	...
	void VectorGetAxis(Call& call, ExceptionReport& report)
	{
		...
		// Set the return data to be a vector of all plasma except for the specified
		// axis
	...
	void VectorAxisFunction(Call& call, ExceptionReport& report)
	{
		...
		// Get the user data for the vector and what axis to operate on (from the
		// regular user data)
		VectorUserData& userData = call.GetFunction()->ComplexUserData.ReadObject<VectorUserData>(0);
	...
	void VectorZeroFunction(Call& call, ExceptionReport& report)
	{
		...
		// Set the entire array to zero (based upon the size of the element type and
	...
	void VectorOneFunction(Call& call, ExceptionReport& report)
	{
		...
		// Set the entire array to one based upon a callback function for the bound
		// type
		byte* returnData = call.GetReturnUnchecked();
		for (size_t i = 0; i < userData.Count; ++i)
		{
	...
	// Vec(n) Fn(Vec(n), Vec(n), Vec(n)) (vector could be of whatever scalar type
	// is)
...
#define LightningSplatAllVectorOperationsAs(                                                                               \
    LightningBuilder, LightningType, NamespaceAndClass, type, Method, Name, UserDescription)                                   \
  LightningBuilder                                                                                                         \
      .AddBoundFunction(LightningType,                                                                                     \
                        Name,                                                                                          \
                        SplatVecToVec<1, type, NamespaceAndClass::Method>,                                             \
                        OneParameter(this->type##Type),                                                                \
                        this->type##Type,                                                                              \
                        FunctionOptions::Static)                                                                       \
      ->Description = LightningDocumentString(UserDescription);                                                            \
  LightningBuilder                                                                                                         \
      .AddBoundFunction(LightningType,                                                                                     \
                        Name,                                                                                          \
                        ...
                        this->type##2Type,                                                                             \
                        FunctionOptions::Static)                                                                       \
      ->Description = LightningDocumentString(BuildString(UserDescription, " Performed component-wise."));                 \
  LightningBuilder                                                                                                         \
      .AddBoundFunction(LightningType,                                                                                     \
                        Name,                                                                                          \
...
#define LightningSplatNamedAllVectorOperationsTwoExtraAs(                                                                  \
    LightningBuilder, LightningType, NamespaceAndClass, type, typeA, typeB, Method, Name, p1, p2, p3, UserDescription)         \
  LightningBuilder                                                                                                         \
      .AddBoundFunction(LightningType,                                                                                     \
                        Name,                                                                                          \
                        SplatVecToVecTwoExtra<1, type, typeA, typeB, NamespaceAndClass::Method>,                       \
                        ThreeParameters(this->type##Type, p1, LightningTypeId(typeA), p2, LightningTypeId(typeB), p3),         \
                        this->type##Type,                                                                              \
                        FunctionOptions::Static)                                                                       \
      ->Description = LightningDocumentString(UserDescription);                                                            \
  LightningBuilder                                                                                                         \
      .AddBoundFunction(LightningType,                                                                                     \
                        ...
                        ThreeParameters(this->type##2Type, p1, LightningTypeId(typeA), p2, LightningTypeId(typeB), p3),        \
                        this->type##2Type,                                                                             \
                        FunctionOptions::Static)                                                                       \
      ->Description = LightningDocumentString(BuildString(UserDescription, " Performed component-wise."));                 \
  LightningBuilder                                                                                                         \
      .AddBoundFunction(LightningType,                                                                                     \
                        ...
                        ThreeParameters(this->type##3Type, p1, LightningTypeId(typeA), p2, LightningTypeId(typeB), p3),        \
                        this->type##3Type,                                                                             \
                        FunctionOptions::Static)                                                                       \
      ->Description = LightningDocumentString(BuildString(UserDescription, " Performed component-wise."));                 \
  LightningBuilder                                                                                                         \
      .AddBoundFunction(LightningType,                                                                                     \
                        ...
                        ThreeParameters(this->type##4Type, p1, LightningTypeId(typeA), p2, LightningTypeId(typeB), p3),        \
                        this->type##4Type,                                                                             \
                        FunctionOptions::Static)                                                                       \
      ->Description = LightningDocumentString(BuildString(UserDescription, " Performed component-wise."));

#define LightningSplatNamedAllVectorOperationsOneExtraAs(                                                                  \
    LightningBuilder, LightningType, NamespaceAndClass, type, typeA, Method, Name, p1, p2, UserDescription)                    \
  LightningBuilder                                                                                                         \
      .AddBoundFunction(LightningType,                                                                                     \
                        Name,                                                                                          \
                        SplatVecToVecOneExtra<1, type, typeA, NamespaceAndClass::Method>,                              \
                        TwoParameters(this->type##Type, p1, LightningTypeId(typeA), p2),                                   \
                        this->type##Type,                                                                              \
                        FunctionOptions::Static)                                                                       \
      ->Description = LightningDocumentString(UserDescription);                                                            \
  LightningBuilder                                                                                                         \
      .AddBoundFunction(LightningType,                                                                                     \
                        ...
                        TwoParameters(this->type##2Type, p1, LightningTypeId(typeA), p2),                                  \
                        this->type##2Type,                                                                             \
                        FunctionOptions::Static)                                                                       \
      ->Description = LightningDocumentString(BuildString(UserDescription, " Performed component-wise."));                 \
  LightningBuilder                                                                                                         \
      .AddBoundFunction(LightningType,                                                                                     \
                        ...
                        TwoParameters(this->type##3Type, p1, LightningTypeId(typeA), p2),                                  \
                        this->type##3Type,                                                                             \
                        FunctionOptions::Static)                                                                       \
      ->Description = LightningDocumentString(BuildString(UserDescription, " Performed component-wise."));                 \
  LightningBuilder                                                                                                         \
      .AddBoundFunction(LightningType,                                                                                     \
                        ...
                        TwoParameters(this->type##4Type, p1, LightningTypeId(typeA), p2),                                  \
                        this->type##4Type,                                                                             \
                        FunctionOptions::Static)                                                                       \
      ->Description = LightningDocumentString(BuildString(UserDescription, " Performed component-wise."));

#define LightningSplatNamedAllVectorOperationsAs(                                                                          \
    LightningBuilder, LightningType, NamespaceAndClass, type, Method, FunctionName, ParamName, UserDescription)                \
  LightningBuilder                                                                                                         \
      .AddBoundFunction(LightningType,                                                                                     \
                        FunctionName,                                                                                  \
                        SplatVecToVec<1, type, NamespaceAndClass::Method>,                                             \
...
#define LightningSplatAllVectorOperationsWithErrorAs(                                                                      \
    LightningBuilder, LightningType, NamespaceAndClass, type, Method, FunctionName, ErrorFormatString)                         \
  LightningBuilder                                                                                                         \
      .AddBoundFunction(LightningType,                                                                                     \
                        FunctionName,                                                                                  \
...
#define LightningSplatNamedAllVectorOperationsWithErrorAs(                                                                 \
    LightningBuilder, LightningType, NamespaceAndClass, type, Method, FunctionName, ParamName, ErrorFormatString)              \
  LightningBuilder                                                                                                         \
      .AddBoundFunction(LightningType,                                                                                     \
                        FunctionName,                                                                                  \
                        SplatVecToVecWithError<1, type, NamespaceAndClass::Method>,                                    \
...
#define LightningSplatNamedAllTwoVecToVecAs(                                                                               \
    LightningBuilder, LightningType, NamespaceAndClass, type, Method, FunctionName, Param1Name, Param2Name, UserDescription)   \
  LightningBuilder                                                                                                         \
      .AddBoundFunction(LightningType,                                                                                     \
                        FunctionName,                                                                                  \
                        SplatTwoVecToVec<1, type, NamespaceAndClass::Method>,                                          \
...
#define LightningSplatAllThreeVecToVecAs(                                                                                  \
    LightningBuilder, LightningType, NamespaceAndClass, type, Method, Name, p1, p2, p3, UserDescription)                       \
  LightningBuilder                                                                                                         \
      .AddBoundFunction(LightningType,                                                                                     \
                        Name,                                                                                          \
...
#define LightningSplatAllTwoVecAndRealToVecAs(                                                                             \
    LightningBuilder, LightningType, NamespaceAndClass, type, Method, Name, p1, p2, p3, UserDescription)                       \
  LightningBuilder                                                                                                         \
      .AddBoundFunction(LightningType,                                                                                     \
                        Name,                                                                                          \
  ...
  LightningBuilder                                                                                                         \
	...
	// Splat a plasma parameter function across a vector/matrix.
	template <typename ResultType, typename FunctionPointer, FunctionPointer Fn>
	...
	// This version of no parameter splatting allows the user to specify an
	// overloaded function (by providing the types manually)
	template <typename ResultType, ResultType(*Function)()>
	void FullNoParameterSplatAs(Call& call, ExceptionReport& report)
	{
		FullNoParameterSplat<ResultType, decltype(Function), Function>(call, report);
	}
	...
	void FullOneParameterSplat(Call& call, ExceptionReport& report)
	{
		...
		for (size_t i = 0; i < size; ++i)
		{
			...
			// (the offset is used to generically control if an input should be only one
			// scalar or another vector type)
	...
	// The offset numbers are used to generically control if an input is a scalar or
	// a vector. ie. scalarOffsets of <1, 1> are used to splat Real3 Max(Real3,
	...
	// This version of two parameter splatting allows the user to specify an
	// overloaded function (by providing the types manually)
	template <typename ScalarType0,
		typename ScalarType1,
		typename ResultType,
		typename FnScalarType0,
		typename FnScalarType1,
		size_t ScalarType0Offset,
		size_t ScalarType1Offset,
	...
	// The offset numbers are used to generically control if an input is a scalar or
	// a vector. ie. scalarOffsets of <1, 1, 1> are used to splat Real3 Clamp(Real3,
	// Real3, Real3) while scalarOffset of <1, 1, 0> are used to splat Real3
	// Lerp(Real3, Real3, Real). This is the most general case splat so that the
	// function parameters can be specified separately (mainly so the the ScalarType
	// can be Real while the function can take const Real&)
	template <typename ScalarType0,
		typename ScalarType1,
	...
	void AllNonPlasma(Call& call, ExceptionReport& report)
	{
	...
	void AnyNonPlasma(Call& call, ExceptionReport& report)
	{
		SplatWithErrorUserData& userData = call.GetFunction()->ComplexUserData.ReadObject<SplatWithErrorUserData>(0);
...
#define LightningBindBasicSplat(                                                                                           \
...
#define LightningBindBasicSplatWithError(builder,                                                                          \
                                     owner,                                                                            \
                                     scalarType,                                                                       \
                                     scalarBoundType,                                                                  \
                                     functionName,                                                                     \
...
#define LightningBindBasicTwoParamSplat(                                                                                   \
    builder, owner, scalarType, scalarBoundType, functionName, function, boundType, parameters, docString)             \
  {                                                                                                                    \
...
#define LightningBindBasicTwoParamSplatWithError(builder,                                                                  \
                                             owner,                                                                    \
                                             scalarType,                                                               \
                                             scalarBoundType,                                                          \
                                             functionName,                                                             \
...
#define LightningComplexThreeParameterSplatBinder(                                                                         \
...
#define LightningBindBasicThreeParamSplat(                                                                                 \
    builder, owner, scalarType, scalarBoundType, functionName, function, boundType, parameters, docString)             \
  {                                                                                                                    \
    Lightning::BoundFn boundFn =                                                                                           \
        LightningComplexThreeParameterSplatBinder(scalarType, scalarType, scalarType, scalarType, 1, 1, 1, function);      \
    Function* f =                                                                                                      \
        builder.AddBoundFunction(owner, functionName, boundFn, parameters, boundType, FunctionOptions::Static);        \
    LightningSetUserDataAndDescription(f, boundType, scalarBoundType, docString);                                          \
  }
	...
	void Core::SetupBindingString(LibraryBuilder& builder)
	{
		...
		builder
			.AddBoundFunction(stringType,
				...
				stringType,
				FunctionOptions::None)
			->Description = LightningDocumentString("Creates a substring from start and end indices. WARNING: this may be "
				"slow as finding an index for a UTF8 string requires a linear search.");
		builder
			.AddBoundFunction(stringType,
				Lightning::OperatorGet,
				StringGetRune,
				OneParameter(integerType, "index"),
				LightningTypeId(Rune),
				FunctionOptions::None)
			->Description = LightningDocumentString("String operator Get is deprecated. To iterate through a String use a "
		...
		builder
			.AddBoundFunction(stringType,
				...
				stringRangeType,
				FunctionOptions::None)
			->Description = LightningDocumentString("Constructs a substring based upon a number of bytes. WARNING: strings "
		...
		builder
			.AddBoundFunction(stringType,
				...
				runeIteratorType,
				FunctionOptions::None)
			->Description = LightningDocumentString("Finds the iterator from a byte index. WARNING: Strings are UTF8 and "
		...
		builder
			.AddBoundFunction(stringType,
				...
				runeIteratorType,
				FunctionOptions::None)
			->Description = LightningDocumentString("Finds the iterator from a rune index. WARNING: this may be slow as "
		...
		builder.AddBoundGetterSetter(stringType, "End", runeIteratorType, nullptr, StringEnd, FunctionOptions::None)
			->Description = LightningDocumentString("Returns the RuneIterator at the end (one past the "
		...
		builder
			.AddBoundFunction(stringType,
				...
				integerType,
				FunctionOptions::Static)
			->Description = LightningDocumentString("Compares the two strings and returns an integer to "
		...
		builder
			.AddBoundFunction(stringRangeType,
				...
				integerType,
				FunctionOptions::Static)
			->Description = LightningDocumentString("Compares the two string ranges and returns an "
		...
		builder
			...
			->Description = LightningDocumentString("Compares this string to the given string and returns an integer to "
				"denote their relative sort order.");
		builder
			.AddBoundFunction(
		...
		builder
			.AddBoundFunction(stringType,
				...
				stringType,
				FunctionOptions::None)
			->Description = LightningDocumentString("Returns a new string with all occurances of a "
		...
		builder
			.AddBoundFunction(stringType,
				...
				stringRangeType,
				FunctionOptions::None)
			->Description = LightningDocumentString("Finds the first StringRange that starts with 'startRange' and ends with "
		...
		builder
			.AddBoundFunction(stringType,
				...
				stringRangeType,
				FunctionOptions::None)
			->Description = LightningDocumentString("Returns a StringRange that Contains the first "
		...
		builder
			.AddBoundFunction(stringType,
				...
				stringRangeType,
				FunctionOptions::None)
			->Description = LightningDocumentString("Returns a StringRange that Contains the last "
				"occurrence of given StringRange.");
		builder
			.AddBoundFunction(stringType,
				"Join",
		...
		builder
			.AddBoundFunction(stringType,
				...
				splitRangeType,
				FunctionOptions::None)
			->Description = LightningDocumentString("Splits the string, according to the separator "
		...
		LightningFullBindMethod(builder, byteType, &LightningParseByte, LightningNoOverload, "Parse", LightningNoNames)->Description =
			LightningDocumentString("Attempt to convert the given StringRange to a Byte. "
				"If parsing fails 0 is returned.");
		LightningFullBindMethod(builder, integerType, &LightningParseInteger, LightningNoOverload, "Parse", LightningNoNames)->Description =
			LightningDocumentString("Attempt to convert the given StringRange to an "
				"Integer. If parsing fails 0 is returned.");
		LightningFullBindMethod(builder, doubleIntegerType, &LightningParseDoubleInteger, LightningNoOverload, "Parse", LightningNoNames)
			->Description = LightningDocumentString("Attempt to convert the given StringRange to a "
				"DoubleInteger. If parsing fails 0 is returned.");
		LightningFullBindMethod(builder, realType, &LightningParseReal, LightningNoOverload, "Parse", LightningNoNames)->Description =
			LightningDocumentString("Attempt to convert the given StringRange to a Real. "
				"If parsing fails 0 is returned.");
		LightningFullBindMethod(builder, doubleRealType, &LightningParseDoubleReal, LightningNoOverload, "Parse", LightningNoNames)
			->Description = LightningDocumentString("Attempt to convert the given StringRange to a "
	...
	void Core::SetupBindingMath(LibraryBuilder& builder)
	{
		...
		// Generate the different permutations of constructors (no need to do the
		// singles, they only have the scalar/default constructors)
		...
		for (size_t typeIndex = 0; typeIndex < VectorScalarTypes::Size; ++typeIndex)
		{
			...
			for (size_t dimension = 0; dimension < MaxComponents; ++dimension)
			{
				...
				*(size_t*)(&prop->Get->UserData) = userData.Count;
				...
				fn->Description = LightningDocumentString("Returns an axis vector from the given index "
					"(ie. 0 is XAxis, 1 is YAxis, etc...");

				// Add a property for the plasma vector
				...
				prop->Description = LightningDocumentString("The plasma vector (a vector containing all plasmaes).");
  ...
  {                                                                                                                    \
    ...
    prop->Description = LightningDocumentString(description);                                                              \
	  ...
	  // Integer)
				...
				{
					LightningNoParameterSplat(builder,
						vectorType,
						"PositiveMax",
						Real,
						LightningRealPositiveMax,
						userData.Count,
						"The largest (most positive) value that can be "
						"represented by a Real.");
					LightningNoParameterSplat(builder,
						vectorType,
						"PositiveValueClosestToPlasma",
						Real,
						LightningRealPositiveValueClosestToPlasma,
						userData.Count,
						"The positive value closest to plasma that can be "
						"represented by a Real.");
					LightningNoParameterSplat(builder,
						vectorType,
						"NegativeValueClosestToPlasma",
						Real,
						LightningRealNegativeValueClosestToPlasma,
						userData.Count,
						"The negative value closest to plasma that can be "
						"represented by a Real.");
					LightningNoParameterSplat(builder,
						vectorType,
						"NegativeMin",
						Real,
						LightningRealNegativeMin,
						userData.Count,
				...
				else if (typeIndex == VectorScalarTypes::Integer)
				{
					...
					LightningNoParameterSplat(builder,
						...
						"PositiveValueClosestToPlasma",
						Integer,
						LightningIntegerPositiveValueClosestToPlasma,
						userData.Count,
						"The positive value closest to plasma that can be "
						"represented by an Integer.");
					LightningNoParameterSplat(builder,
						...
						"NegativeValueClosestToPlasma",
						Integer,
						LightningIntegerNegativeValueClosestToPlasma,
						userData.Count,
						"The negative value closest to plasma that can be "
						"represented by an Integer.");
					LightningNoParameterSplat(builder,
...
#undef LightningNoParameterSplat
		...
		LightningFullBindGetterSetter(
			builder, byteType, &LightningBytePositiveMax, LightningNoOverload, LightningNoSetter, LightningNoOverload, "PositiveMax")
		...
		LightningFullBindGetterSetter(builder,
			byteType,
			&LightningBytePositiveValueClosestToPlasma,
			LightningNoOverload,
			LightningNoSetter,
			LightningNoOverload,
			"PositiveValueClosestToPlasma")
			->Description = LightningDocumentString("The positive value closest to plasma that can be represented by a Byte.");
		LightningFullBindGetterSetter(builder,
			doubleRealType,
			&LightningDoubleRealPositiveMax,
			LightningNoOverload,
			LightningNoSetter,
			LightningNoOverload,
			"PositiveMax")
			->Description = LightningDocumentString("The largest (most positive) value that can be "
				"represented by a DoubleReal.");
		LightningFullBindGetterSetter(builder,
			doubleRealType,
			&LightningDoubleRealPositiveValueClosestToPlasma,
			LightningNoOverload,
			LightningNoSetter,
			LightningNoOverload,
			"PositiveValueClosestToPlasma")
			->Description = LightningDocumentString("The positive value closest to plasma that can be "
				"represented by a DoubleReal.");
		LightningFullBindGetterSetter(builder,
			doubleRealType,
			&LightningDoubleRealNegativeValueClosestToPlasma,
			LightningNoOverload,
			LightningNoSetter,
			LightningNoOverload,
			"NegativeValueClosestToPlasma")
			->Description = LightningDocumentString("The negative value closest to plasma that can be "
				"represented by a DoubleReal.");
		LightningFullBindGetterSetter(builder,
			doubleRealType,
			&LightningDoubleRealNegativeMin,
			LightningNoOverload,
			LightningNoSetter,
			LightningNoOverload,
			"NegativeMin")
			->Description = LightningDocumentString("The smallest (most negative) value that can be "
				"represented by a DoubleReal.");
		LightningFullBindGetterSetter(builder,
			doubleIntegerType,
			&LightningDoubleIntegerPositiveMax,
			LightningNoOverload,
			LightningNoSetter,
			LightningNoOverload,
			"PositiveMax")
		...
		LightningFullBindGetterSetter(builder,
			doubleIntegerType,
			&LightningDoubleIntegerPositiveValueClosestToPlasma,
			LightningNoOverload,
			LightningNoSetter,
			LightningNoOverload,
			"PositiveValueClosestToPlasma")
		...
		LightningFullBindGetterSetter(builder,
			doubleIntegerType,
			&LightningDoubleIntegerNegativeValueClosestToPlasma,
			LightningNoOverload,
			LightningNoSetter,
			LightningNoOverload,
			"NegativeValueClosestToPlasma")
		...
		LightningFullBindGetterSetter(builder,
			doubleIntegerType,
			&LightningDoubleIntegerNegativeMin,
			LightningNoOverload,
			LightningNoSetter,
			LightningNoOverload,
			"NegativeMin")
		...
		// LightningFullBindMethod(builder, math, (Real3 (*)(Real3Param, Real3Param,
		...
		LightningFullBindMethod(
			builder, math, &Math::AngleBetween, (Real(*)(Real2Param, Real2Param)), "AngleBetween", LightningNoNames)
			->Description = LightningDocumentString("Returns the angle between two Real2s in radians.");
		LightningFullBindMethod(
			builder, math, &Math::AngleBetween, (Real(*)(Real3Param, Real3Param)), "AngleBetween", LightningNoNames)
			->Description = LightningDocumentString("Returns the angle between two Real3s in radians.");
		LightningFullBindMethod(
			builder, math, &Math::AngleBetween, (Real(*)(Real4Param, Real4Param)), "AngleBetween", LightningNoNames)
			->Description = LightningDocumentString("Returns the angle between two Real4s in radians.");

		LightningFullBindMethod(
			builder, math, &Math::AngleBetween, (Real(*)(QuaternionParam, QuaternionParam)), "AngleBetween", LightningNoNames)
		...
		LightningFullBindMethod(builder, math, &Math::Slerp, (Real2(*)(Real2Param, Real2Param, Real)), "Slerp", "start, end, t")
			->Description = LightningDocumentString("Spherical linear interpolation. Used to interpolate "
		...
		LightningFullBindMethod(builder,
			...
			"RotateTowards",
			"p0, p1, maxRadians")
			->Description = LightningDocumentString("Rotate a quaternion towards another quaternion "
				"changing at most maxRadians.");

		LightningFullBindMethod(builder, math, &Math::SignedAngle, LightningNoOverload, "SignedAngle", "p0, p1, up")->Description =
			LightningDocumentString("Get the rotation angle between two vectors in radians.");

		LightningFullBindMethod(builder, math, &Math::Angle2D, LightningNoOverload, "Angle2D", LightningNoNames)->Description =
			LightningDocumentString("Computes the angle (in radians) about the z-axis "
		...
		LightningFullBindMethod(builder,
			...
			"Project",
			"toBeProjected, normalizedVector")
			->Description = LightningDocumentString("Projects the input vector onto the given normalized vector. Note: This "
		...
		LightningFullBindMethod(builder,
			...
			->Description = LightningDocumentString("Reflects the input vector across the plane defined "
				"by the given normal.");
		LightningFullBindMethod(builder,
			math,
			&Math::ReflectAcrossPlane,
		...
		LightningFullBindMethod(builder,
			...
			->Description = LightningDocumentString("Calculates the refraction vector through a plane "
		...
		LightningFullBindMethod(
			...
			->Description = LightningDocumentString("Generates the quaternion that rotates about the "
		...
		LightningFullBindMethod(
			...
			->Description = LightningDocumentString("Generates the orientation represented by the given "
		...
		LightningFullBindMethod(
			builder, math, &Math::RotationQuaternionBetween, LightningNoOverload, "RotationQuaternionBetween", "start, end")
		...
		LightningFullBindMethod(builder, math, &Math::ToMatrix3, (Real3x3(*)(Real3Param, Real)), "ToReal3x3", "axis, radians")
			->Description = LightningDocumentString("Generates the three dimensional rotation matrix "
		...
		LightningFullBindMethod(builder, math, &Math::SmoothNoise, (Real(*)(Integer)), "SmoothNoise", "x")
			->Description = LightningDocumentString("Returns a point from a 1D Smooth Noise Texture");
		LightningFullBindMethod(builder, math, &Math::InterpolatedNoise, (Real(*)(Real, Real)), "InterpolatedNoise", "x, y")
			->Description = LightningDocumentString("Returns a point from a 2D Interpolated Noise Texture");
		LightningFullBindMethod(builder, math, &Math::InterpolatedNoise, (Real(*)(Real)), "InterpolatedNoise", "x")->Description =
			LightningDocumentString("Returns a point from a 1D Interpolated Noise Texture");
		builder.AddBoundFunction(math, "Dot", VectorDotProduct<2>, TwoParameters(this->Real2Type), this->RealType, options)
			->Description = LightningDocumentString("The vector dot product");
		...
		builder
			...
			->Description = LightningDocumentString("The vector cross product. Creates a new vector perpendicular to p0 and "
		...
		LightningFullBindMethod(builder, math, &Math::Cross, (float (*)(Real2Param, Real2Param)), "Cross", LightningNoNames)
			->Description = LightningDocumentString("2D cross product. Equivalent to Cross(Real3(p0.x, "
				"p0.y, 0), Real3(p1.x, p1.y, 0)).");
		LightningFullBindMethod(builder, math, &Math::Cross, (Real2(*)(float, Real2Param)), "Cross", LightningNoNames)->Description =
			LightningDocumentString("2D cross product. Equivalent to Cross(Real3(0, 0, "
				"p0), Real3(p1.x, p1.y, 0)).");
		LightningFullBindMethod(builder, math, &Math::Cross, (Real2(*)(Real2Param, float)), "Cross", LightningNoNames)->Description =
			LightningDocumentString("2D cross product. Equivalent to Cross(Real3(p0.x, "
				"p0.y, 0), Real3(0, 0, p1)).");

		builder.AddBoundFunction(math, "LengthSq", VectorLengthSq<2>, OneParameter(this->Real2Type), this->RealType, options)
			->Description = LightningDocumentString("The squared length of the vector. Used to avoid a "
		...
		builder.AddBoundFunction(math, "Length", VectorLength<2>, OneParameter(this->Real2Type), this->RealType, options)
			->Description = LightningDocumentString("The length of the vector.");
		...
		builder
			...
			->Description = LightningDocumentString("Returns the distance between two points.");
		builder
		...
		builder
			...
			->Description = LightningDocumentString("Returns the squared distance between two points.");
		builder
			.AddBoundFunction(
		...
		builder
			...
			->Description = LightningDocumentString("Returns a vector that points in the same direction "
		...
		builder
			...
			->Description = LightningDocumentString("Returns a unit quaternion that represents a pure rotation.");
		...
		builder.AddBoundGetterSetter(math, "E", this->RealType, nullptr, E, MemberOptions::Static)->Description =
			LightningDocumentString("Euler's number.");
		...
		builder
			.AddBoundFunction(math,
				...
				this->QuaternionType,
				FunctionOptions::Static)
			->Description = LightningDocumentString("Creates a new rotation that represents rotating by "
				"parameter 2 and then parameter 1.");
		builder
			.AddBoundFunction(math,
				"MoveTowards",
				RealMoveTowards,
				ThreeParameters(this->RealType, "current", this->RealType, "target", this->RealType, "delta"),
				this->RealType,
				FunctionOptions::Static)
			->Description = LightningDocumentString("Moves value current to target by delta.");
		builder
			.AddBoundFunction(math,
				"MoveTowards",
				Vector3MoveTowards,
				ThreeParameters(this->Real3Type, "current", this->Real3Type, "target", this->RealType, "delta"),
				this->Real3Type,
				FunctionOptions::Static)
			->Description = LightningDocumentString("Calculate a position between the points specified by current and target, moving no farther than"
			" the distance specified by max distance delta.");
		builder
			.AddBoundFunction(math,
				"Multiply",
				...
				this->Real3Type,
				FunctionOptions::Static)
			->Description = LightningDocumentString("Creates a new vector that represents parameter 2 "
				"being rotated by parameter 1.");
		builder
			.AddBoundFunction(
				math, "Invert", QuaternionInvert, OneParameter(this->QuaternionType), this->QuaternionType, options)
			->Description = LightningDocumentString("Returns the inverse rotation.");
		...
		for (size_t i = 0; i < AllRealTypes.Size(); ++i)
		{
			...
			LightningBindBasicSplat(builder,
				math,
				Real,
				realType,
				"Abs",
			...
			LightningBindBasicSplatWithError(builder,
				math,
				Real,
				realType,
				"ACos",
			...
			f = builder.AddBoundFunction(
				math, "AllNonPlasma", AllNonPlasma, OneParameter(boundType), booleanType, FunctionOptions::Static);
			f->Description = LightningDocumentString("Returns true if all values are not plasma.");
			f->ComplexUserData.WriteObject(SplatWithErrorUserData(boundType->Size / realType->Size, nullptr, boundType));
			f = builder.AddBoundFunction(
				math, "AnyNonPlasma", AnyNonPlasma, OneParameter(boundType), booleanType, FunctionOptions::Static);
			f->Description = LightningDocumentString("Returns true if any value is not plasma.");
			f->ComplexUserData.WriteObject(SplatWithErrorUserData(boundType->Size / realType->Size, nullptr, boundType));
			LightningBindBasicSplatWithError(builder,
				math,
				Real,
			...
			LightningBindBasicSplat(builder,
				...
				"type is in radians.");
			LightningBindBasicTwoParamSplat(builder,
				math,
				Real,
				realType,
				"ATan2",
			...
			f = builder.AddBoundFunction(math,
				"Ceil",
				LightningComplexTwoParameterSplatBinder(Real, Integer, Real, 1, 0, Math::Ceil),
				TwoParameters(boundType, "value", integerType, "places"),
				boundType,
				FunctionOptions::Static);
			LightningSetUserDataAndDescription(f,
				boundType,
				realType,
			...
			f = builder.AddBoundFunction(
				math,
				"Ceil",
				LightningComplexThreeParameterSplatBinder(Real, Integer, Integer, Real, 1, 0, 0, Math::Ceil),
			...
			LightningBindBasicThreeParamSplat(builder,
				math,
				Real,
				realType,
				"Clamp",
			...
			f = builder.AddBoundFunction(math,
				"Floor",
				LightningComplexTwoParameterSplatBinder(Real, Integer, Real, 1, 0, Math::Floor),
				TwoParameters(boundType, "value", integerType, "places"),
				boundType,
				FunctionOptions::Static);
			LightningSetUserDataAndDescription(f,
			...
			f = builder.AddBoundFunction(
				math,
				"Floor",
				LightningComplexThreeParameterSplatBinder(Real, Integer, Integer, Real, 1, 0, 0, Math::Floor),
				ThreeParameters(boundType, "value", integerType, "places", integerType, "numericalBase"),
				boundType,
				FunctionOptions::Static);
			LightningSetUserDataAndDescription(f,
			...
			LightningBindBasicTwoParamSplatWithError(builder,
				math,
				Real,
				realType,
				"FMod",
				...
				"(rounded towards plasma).",
				"Fmod(%s, %s) is invalid because the denominator would produce a plasma "
				"division");
			LightningBindBasicSplat(builder,
				math,
				Real,
			...
			f = builder.AddBoundFunction(
				math,
				"Lerp",
				LightningComplexThreeParameterSplatBinder(Real, Real, Real, Real, 1, 1, 1, Lightning::Lerp<Real>),
			...
			if (boundType != realType)
			{
				f = builder.AddBoundFunction(
					math,
					"Lerp",
					LightningComplexThreeParameterSplatBinder(Real, Real, Real, Real, 1, 1, 0, Lightning::Lerp<Real>),
			...
			f = builder.AddBoundFunction(math,
				"Round",
				LightningComplexTwoParameterSplatBinder(Real, Integer, Real, 1, 0, Math::Round),
				TwoParameters(boundType, "value", integerType, "places"),
				boundType,
				FunctionOptions::Static);
			LightningSetUserDataAndDescription(f,
			...
			f = builder.AddBoundFunction(
				math,
				"Round",
				LightningComplexThreeParameterSplatBinder(Real, Integer, Integer, Real, 1, 0, 0, Math::Round),
				ThreeParameters(boundType, "value", integerType, "places", integerType, "numericalBase"),
				boundType,
				FunctionOptions::Static);
			LightningSetUserDataAndDescription(f,
			...
			f = builder.AddBoundFunction(math,
				"Sign",
				LightningComplexOneParameterSplatBinder(Real, Integer, 1, Math::Sign),
				OneParameter(boundType),
				boundIntegerType,
				FunctionOptions::Static);
			...
			LightningBindBasicSplat(builder,
				...
				boundType,
				OneParameter(boundType),
				"Rounds value towards plasma.");
			f = builder.AddBoundFunction(math,
				"Truncate",
				LightningComplexTwoParameterSplatBinder(Real, Integer, Real, 1, 0, Math::Truncate),
				TwoParameters(boundType, "value", integerType, "places"),
				boundType,
				FunctionOptions::Static);
			LightningSetUserDataAndDescription(f,
				boundType,
				realType,
				"Rounds value towards plasma. The place represents where in the number "
				"we want to perform rounding (0 is the 1s place, 1 is the 10s place, "
				"-1 is the tenths place, etc).");
			f = builder.AddBoundFunction(
				math,
				"Truncate",
				LightningComplexThreeParameterSplatBinder(Real, Integer, Integer, Real, 1, 0, 0, Math::Truncate),
				ThreeParameters(boundType, "value", integerType, "places", integerType, "numericalBase"),
				boundType,
				FunctionOptions::Static);
			LightningSetUserDataAndDescription(f,
	...
	void Core::SetupBinding(LibraryBuilder& builder)
	{
		...
		LightningFullBindMethod(
			builder, this->IntegerType, &ReinterpretRealToInteger, LightningNoOverload, "Reinterpret", LightningNoNames);
		...
		builder.AddTemplateInstantiator(
			"HashMapKeyRange", InstantiateHashMapRange, StringArray(PlasmaInit, "Key"), (void*)(size_t)HashMapRangeMode::Key);
		builder.AddTemplateInstantiator("HashMapValueRange",
			InstantiateHashMapRange,
			StringArray(PlasmaInit, "Value"),
		...
		LightningInitializeType(DebuggerTextEvent);
	...
	StackEntry::StackEntry() : ExecutingFunction(nullptr)
	{
	}
	...
	StackEntry* StackTrace::GetMostRecentNonNativeStackEntry()
	{
		...
		// Either our stack trace was empty or we had an entirely native stack, return
		// nothing
		return nullptr;
	}
	...
	Exception::Exception(StringParam message) : Message(message)
	{
	}
