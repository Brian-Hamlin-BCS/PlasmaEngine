######################################
### PlasmaWritePVSHeader(<file-path> <optional-text>)
######################################

function(PlasmaWritePVSHeader TARGET_FILE OPTIONAL_TEXT)
...
######################################
### PlasmaGenerateFolderUnityFile(<target> <project-path> <rel-path-to-folder> <files-in-folder>)
######################################

function(PlasmaGenerateFolderUnityFile TARGET_NAME PROJECT_DIRECTORY SUB_FOLDER_PATH FILE_LIST)
  ...
  PlasmaWritePVSHeader(${TMP_UNITY_FILE} "// generated unity cpp file\n\n")

  # find the PCH and include it as the first thing in the unity file
  PlasmaRetrieveTargetPCH(${TARGET_NAME} PCH_FILE)

  if (PCH_FILE)
    file(APPEND ${TMP_UNITY_FILE} "#include <${PCH_FILE}.hpp>\n\n")
	
	PlasmaPCHUse("${PCH_FILE}.hpp" "${FOLDER_UNITY_FILE}")
  endif()
...
######################################
### PlasmaGenerateFolderUnityFilesForTarget(<target> <project-path> [<folder-to-exclude> ...])
######################################

function(PlasmaGenerateFolderUnityFilesForTarget TARGET_NAME TARGET_DIRECTORY)

  PlasmaGatherSubfolders(${TARGET_DIRECTORY} ALL_FOLDERS)
  ...
  foreach(CUR_FOLDER ${ALL_FOLDERS})
    ...
    PlasmaGenerateFolderUnityFile(${TARGET_NAME} "${TARGET_DIRECTORY}" "${CUR_FOLDER}" "${FILES_IN_FOLDER}")
