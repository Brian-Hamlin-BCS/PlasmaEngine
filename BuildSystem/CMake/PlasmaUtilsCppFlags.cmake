...
######################################
### PlasmaSetBuildFlagsMSVC(<target>)
######################################

function(PlasmaSetBuildFlagsMSVC TARGET_NAME)
...
######################################
### PlasmaSetBuildFlagsClang(<target>)
######################################
				
function(PlasmaSetBuildFlagsClang TARGET_NAME)
	...
	if(NOT PLASMA_CMAKE_PLATFORM_ANDROID AND NOT PLASMA_CMAKE_PLATFORM_WINDOWS)
	...
	if(PLASMA_SUBMODULE_PREFIX_PATH)
		target_compile_options(${TARGET_NAME} PRIVATE "--system-header-prefix=\"${CMAKE_SOURCE_DIR}/${PLASMA_SUBMODULE_PREFIX_PATH}/Code/ThirdParty\"")
	else()
...
######################################
### PlasmaSetBuildFlagsGCC(<target>)
######################################

function(PlasmaSetBuildFlagsGCC TARGET_NAME)
...
######################################
### PlasmaSetBuildFlags(<target>)
######################################

function(PlasmaSetBuildFlags TARGET_NAME)
    ...
    PlasmaSetBuildFlagsMSVC(${TARGET_NAME})

	endif()

	if (PLASMA_CMAKE_COMPILER_CLANG)

        PlasmaSetBuildFlagsClang(${TARGET_NAME})

	endif()

	if (PLASMA_CMAKE_COMPILER_GCC)

        PlasmaSetBuildFlagsGCC(${TARGET_NAME})
